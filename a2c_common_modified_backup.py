        ...
        for n in range(self.horizon_length):
            ...
            if ppo_cycle_debug and n < 5:
                ...
                if n == 0 and not initial_state_logged:
                    ...
                    print("[DNNE_DEBUG] === PPO TRAINING LOOP START ===")
                    print(f"[PPO_INITIAL] First observation: {self.obs['obs'][0][:4].tolist()}")
                    
                    # Try to access network weights if possible
                    if hasattr(self.model, 'a2c_network'):
                        network = self.model.a2c_network
                        if hasattr(network, 'actor_mlp'):
                            # Get first layer of actor
                            first_layer = None
                            for layer in network.actor_mlp:
                                if hasattr(layer, 'weight'):
                                    ...
                                    break
                            if first_layer:
                                print(f"[PPO_INITIAL] Actor first layer weights: {first_layer.weight[0][:4].tolist()}")
                                print(f"[PPO_INITIAL] Actor first layer bias: {first_layer.bias[:4].tolist()}")
                        
                        if hasattr(network, 'mu') and hasattr(network.mu, 'weight'):
                            print(f"[PPO_INITIAL] Mu layer weights: {network.mu.weight[0][:4].tolist()}")
                            print(f"[PPO_INITIAL] Mu layer bias: {network.mu.bias.tolist()}")
                        
                        if hasattr(network, 'sigma') and hasattr(network.sigma, 'weight'):
                            print(f"[PPO_INITIAL] Sigma vals: {network.sigma.tolist() if network.sigma.numel() <= 4 else network.sigma[:4].tolist()}")
                ...
                print(f"[PPO_CYCLE] Step {n + 1}: action={action:.4f}, value={value:.4f}, reward={reward:.4f}")
