import rl_games_dnne.envs.test
from rl_games_dnne.common import wrappers
from rl_games_dnne.common import tr_helpers
from rl_games_dnne.envs.brax import create_brax_env
from rl_games_dnne.envs.envpool import create_envpool
from rl_games_dnne.envs.cule import create_cule
import gym
...
def create_slime_gym_env(**kwargs):
    ...
    from rl_games_dnne.envs.slimevolley_selfplay import SlimeVolleySelfplay
    name = kwargs.pop('name')
    limit_steps = kwargs.pop('limit_steps', False)
...
def create_connect_four_env(**kwargs):
    from rl_games_dnne.envs.connect4_selfplay import ConnectFourSelfPlay
    name = kwargs.pop('name')
    limit_steps = kwargs.pop('limit_steps', False)
    self_play = kwargs.pop('self_play', False)
    if self_play:
...
def create_smac(name, **kwargs):
    from rl_games_dnne.envs.smac_env import SMACEnv
    frames = kwargs.pop('frames', 1)
...
def create_multiwalker_env(**kwargs):
    from rl_games_dnne.envs.multiwalker import MultiWalker
    env = MultiWalker('', **kwargs) 

    return env

def create_diambra_env(**kwargs):
    from rl_games_dnne.envs.diambra.diambra import DiambraEnv
